...
uint8_t ultAngDireito = 50;
uint8_t ultAngEsquerda = 50;
...
void moveServoE(int ang){
  if(ang > ultAngEsquerda){
    ...
    for(int angulo = ServoEsquerdo; angulo >= ang; angulo--) {
      pwm.setPWM(ServoEsquerdo, 0, converterAnguloParaPulso(angulo));
      Serial.println(angulo);
      delay(20);
      ultAngEsquerda = angulo;
...
void loop() {  
  ...
  //   // Envia o sinal PWM para os respectivos canais
  //   pwm.setPWM(ServoGarra, 0, converterAnguloParaPulso(angulo));
  //   pwm.setPWM(ServoDireito, 0, converterAnguloParaPulso(180 - angulo)); // Move invertido
  //   pwm.setPWM(ServoEsquerdo, 0, converterAnguloParaPulso(angulo));
  //   pwm.setPWM(ServoBase, 0, converterAnguloParaPulso(201));
  //   delay(50); // Pequena pausa para dar tempo ao servo de se mover
  // }
  ...
  // for (int angulo = 120; angulo >= 50; angulo -= 10) {
  //   pwm.setPWM(ServoGarra, 0, converterAnguloParaPulso(angulo));
  //   pwm.setPWM(ServoDireito, 0, converterAnguloParaPulso(180 - angulo));
  //   pwm.setPWM(ServoEsquerdo, 0, converterAnguloParaPulso(angulo));
  //   pwm.setPWM(ServoBase, 0, converterAnguloParaPulso(180));
  //   delay(50);
  // }
